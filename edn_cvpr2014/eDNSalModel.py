...
class EDNSaliencyModel(object): 
    ...
    def __init__(self, descriptions, svmModel, biasToCntr=False, insize=(512, 384)):  
    ...
    def saliency(self, img, normalize=True): 
        ...
        imgSize = img.shape[:2]  
        rescale_factor = 0.5*self.EDN_INSIZE[0]/max(imgSize) + \
                         0.5*self.EDN_INSIZE[1]/min(imgSize) 

        # single image: 
        # img = misc.imresize(image, rescale_factor, 'bicubic') 
        
        # image sequence (or single image) 
        scaledImg = np.zeros([np.int(a*rescale_factor) for a in imgSize] + [img.shape[2]])
